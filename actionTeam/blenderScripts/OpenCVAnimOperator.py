...
class OpenCVAnimOperator(bpy.types.Operator):
    ...
    landmark_model_path = "C:\\Users\\Radu Chirila\\Downloads\\lbfmodel.yaml"  # Windows
        ...
        if event.type == 'TIMER':
            ...
            if type(faces) is numpy.ndarray and faces.size > 0:
                ...
                for mark in landmarks:
                    ...
                    # head rotation
                    bones["head_fk"].rotation_euler[0] = self.smooth_value("h_x", 5, (
                                self.rotation_vector[0] - self.first_angle[0])) / 1  # Up/Down
                    bones["head_fk"].rotation_euler[2] = self.smooth_value("h_y", 5, -(
                                self.rotation_vector[1] - self.first_angle[1])) / 1.5  # Rotate
                    bones["head_fk"].rotation_euler[1] = self.smooth_value("h_z", 5, (
                    ...
                    # eyelids
                    l_open = self.smooth_value("e_l", 2,
                                               self.get_range("l_open", -numpy.linalg.norm(shape[48] - shape[44])))
                    r_open = self.smooth_value("e_r", 2,
                                               self.get_range("r_open", -numpy.linalg.norm(shape[41] - shape[39])))
